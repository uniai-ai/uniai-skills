...
```
Create a 1-minute video introducing HeyGen to knowledge workers in Japan.
Explain the features of the Talking Avatar models, and include three concrete use cases.
The selected presenter narrates.

CRITICAL ON-SCREEN TEXT (display literally):
- "HeyGen Avatar V"
- "Create videos in 3 minutes"
...
Note: Even when the script and on-screen text are in another language, the STYLE block and scene labels stay in English — these are Video Agent directives, not viewer-facing content.
