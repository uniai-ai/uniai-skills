---
name: uniai-image
description: Generate images for the current task (website/app assets, product or UI mockups, hero images, logos, illustrations, photorealistic images, infographics, concept art) from a text prompt — optionally guided by reference images (image-to-image: variation, style, or character consistency). Use when the user asks to create, generate, make, or draw an image, or to make a variation of / a new image in the style of an existing or uploaded image (e.g. "生成一张图片", "画一个……", "做张海报", "生成一张类似的图", "同款风格再来一张", "make an image of a cat", "a variation of this image", "in the style of this photo"). For SHORT FILMS / dramas / multi-shot pipelines (短剧/短片/微电影/分镜) do NOT use this one-off tool — build a UniAI canvas instead (`uniai canvas` CLI / uniai-cli skill): it chains shots with shared character/scene references, idempotent re-runs and one estimate gate.
---
